/************************************************************************
 * VicThree Defence — CDS Geography Mock Test : GRADING BACKEND
 * ---------------------------------------------------------------------
 * This Google Apps Script web app grades submissions SERVER-SIDE so the
 * answer key never reaches the browser. It also logs every submission to
 * a Google Sheet.
 *
 * ┌──────────────────────────────────────────────────────────────────┐
 ...
 * │ 2. Delete the default code, paste THIS entire file in.             │
 * │ 3. Click Save (disk icon). Name it e.g. "VicThree CDS Grader".     │
 * │ 4. (Optional) Run the function `setup` once from the editor to     │
 * │    auto-create the results spreadsheet, then Authorize when asked. │
 ...
 * │ 7. Open docs/app.js and paste it into:                             │
 * │        const CONFIG = { BACKEND_URL: "...paste here..." };         │
 * │                                                                    │
 * │ NOTE: every time you edit this script, you must Deploy ▸           │
 * │ Manage deployments ▸ edit ▸ New version, OR create a new           │
 * │ deployment, for changes to go live.                                │
 * └──────────────────────────────────────────────────────────────────┘
 ************************************************************************/
...
/* ============================================================
 * doPost — receives { name, roll, answers }, grades, logs, returns JSON.
 * ============================================================ */
function doPost(e) {
  try {
    ...
    var keys = Object.keys(ANSWER_KEY).sort(function (a, b) { return Number(a) - Number(b); });
    keys.forEach(function (n) {
      max++;
      var key = ANSWER_KEY[n];
      var chosen = answers[n] != null ? String(answers[n]).toLowerCase() : null;
      var isCorrect = chosen != null && chosen === key.correct;
      if (isCorrect) total++;
      results.push({
        n: Number(n),
        chosen: chosen,
    ...
    // Log to the spreadsheet (name, roll, score, timestamp).
    logSubmission(name, roll, total, max);
...
function doGet(e) {
  return jsonOutput({ ok: true, message: "VicThree CDS grader is running. Use POST to submit." });
}

/* ============================================================
 * Logging — appends one row per submission to a Google Sheet.
 * ============================================================ */
function logSubmission(name, roll, score, max) {
  var ss = getSpreadsheet();
  var sheet = ss.getSheetByName(SHEET_NAME);
  if (!sheet) {
    sheet = ss.insertSheet(SHEET_NAME);
    sheet.appendRow(["Timestamp", "Name", "Roll", "Score", "Out of"]);
  }
  sheet.appendRow([new Date(), name, roll, score, max]);
}
...
function getSpreadsheet() {
  ...
  var ss = SpreadsheetApp.create("VicThree CDS — Mock Test Results");
...
/* Optional: run once from the editor to create the spreadsheet up-front
   and trigger the authorization prompt. After running, check the script
   log / your Google Drive for "VicThree CDS — Mock Test Results". */
function setup() {
  var ss = getSpreadsheet();
